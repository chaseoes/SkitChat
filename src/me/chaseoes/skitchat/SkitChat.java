...
public class SkitChat extends JavaPlugin {
	...
	public void onEnable() {
		...
		PlayerData.getInstance().getInstance().setup(this);
	...
	public boolean onCommand(CommandSender cs, Command cmnd, String string, String[] strings) {
				...
				if (!(strings.length > 1)) {
					if (strings.length == 1) {
						...
						if (!(target == null)) {
							if (!target.hasPermission("skitchat.nopm")) {
								if (!pmtoggled.contains(target.getName())) {
									...
									PlayerData.getInstance().setPMTarget((Player) cs, target);
						...
						if (pming.contains(cs.getName())) {
							...
							PlayerData.getInstance().setPMTarget((Player) cs, null);
		...
		if (cmnd.getName().equalsIgnoreCase("reply")) {
			...
			for (String arg : strings)
				sb.append(arg + " ");
			Player player = Bukkit.getPlayer(cs.getName());
			String target = PlayerData.getInstance().getLastPMFrom(player);
		...
		if (cmnd.getName().equalsIgnoreCase("chat")) {
			if (strings.length > 0) {
				...
				if (strings[0].equalsIgnoreCase("spy")) {
					if (cs.hasPermission("skitchat.spy")) {
						if (strings.length == 1) {
							if (PlayerData.getInstance().isSpying((Player) cs)) {
								PlayerData.getInstance().setSpying((Player) cs, false);
								cs.sendMessage("§aYou are no longer spying on private messages.");
							} else {
								PlayerData.getInstance().setSpying((Player) cs, true);
						...
						if (strings.length > 2) {
							if (!(target == null)) { 
								if (!pmtoggled.contains(target.getName())) {
									...
									PlayerData.getInstance().setLastPMFrom((Player) cs, target);
									...
									for (Player players : online) {
										if (players.hasPermission("skitchat.spy") && (!players.getName().equalsIgnoreCase(target.getName())) && (!target.getName().equalsIgnoreCase(players.getName()))) {
											if (PlayerData.getInstance().isSpying(players)) {
