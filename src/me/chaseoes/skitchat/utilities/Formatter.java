...
public class Formatter {
	...
	public String pm(String string, String message, Player cs) {
		Player player = cs;
		String formatted = Utilities.getInstance().colorize(string.replace("%n", player.getName())
						.replace("%p", Utilities.getInstance().getPrefix(player))
						.replace("%s", Utilities.getInstance().getSuffix(player))
						.replace("%dn", player.getDisplayName())
						.replace("%g",  Utilities.getInstance().getGroup(player)))
						.replace("%m", message);
		return formatted;
	}
	...
	public String friend(String format, String message, PlayerChatEvent event) {
		Player player = event.getPlayer();
		String group = Utilities.getInstance().getGroup(player);
		String msg;
		if (player.hasPermission("skitchat.colors")) {
